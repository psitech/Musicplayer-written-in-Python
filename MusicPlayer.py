# Python 3.13.9, customTkinter 5.2.2, pygame 2.6.1, mutagen 1.47.0
import threading
...
class MusicPlayer(ctk.CTk):
    def __init__(self):
        ...
        self.current_time_label = ctk.CTkLabel(self.progress_frame, text="00:00", font=("Segoe UI", 12))
        self.current_time_label.pack(side="left", padx=10)
        
        self.seek_slider = ctk.CTkSlider(self.progress_frame, from_=0, to=100, height=16,
                                         command=self.slider_event)
        self.seek_slider.set(0)
        self.seek_slider.pack(side="left", fill="x", expand=True, padx=10)
        
        self.total_time_label = ctk.CTkLabel(self.progress_frame, text="00:00", font=("Segoe UI", 12))
        ...
        btn_style = {"corner_radius": 10, "height": 40, "font": ("Segoe UI", 14, "bold")}
        
        self.btn_open = ctk.CTkButton(self.controls_frame, text="Open music folder", 
                                      command=self.start_folder_scan, **btn_style)
        self.btn_open.grid(row=0, column=0, padx=10, pady=10)
        
        self.btn_prev = ctk.CTkButton(self.controls_frame, text="PREV", width=100, 
                                      command=self.prev_track, **btn_style)
        self.btn_prev.grid(row=0, column=2, padx=5, pady=10)
        
        self.btn_play = ctk.CTkButton(self.controls_frame, text="PLAY/PAUSE", 
                                      command=self.toggle_play, **btn_style)
        self.btn_play.grid(row=0, column=3, padx=5, pady=10)

        self.btn_stop = ctk.CTkButton(self.controls_frame, text="STOP", width=100, 
                                      command=self.stop_music, **btn_style)
        self.btn_stop.grid(row=0, column=4, padx=5, pady=10)

        self.btn_next = ctk.CTkButton(self.controls_frame, text="NEXT", width=100, 
                                      command=self.next_track, **btn_style)
        self.btn_next.grid(row=0, column=5, padx=5, pady=10)
        
        self.status_label = ctk.CTkLabel(self.controls_frame, text="0 tracks loaded", font=("Segoe UI", 12))
    ...
    def monitor_playback(self): # stable updates for seek bar and song completion
        if self.is_playing and not self.is_paused:
            ...
            if relative_pos == -1: 
                ...
                # end of track detection (threshold of 0.5s)
